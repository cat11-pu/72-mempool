...
def main() -> int:
    ...
    before = {name: pool.deref(handle)["value"] for name, handle in sorted(handles.items())}
    compacted = pool.compact()
    after = {name: pool.deref(handle)["value"] for name, handle in sorted(handles.items())}
    ...
    print("不变量（整理后句柄仍指向原数据） =", spec["handle_invariant"])
